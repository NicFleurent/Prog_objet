...
public class CIDate {
	...
	public CIDate(int _annee, int _mois, int _jour)
	{
		setAnnee(_annee);
		setMois(_mois);
		setJour(_jour);
	...
	private boolean validerDateComplete(int _jour)
	{
		...
		switch (mois)
		{
		case 1:	case 3:	case 5:	case 7:	case 8: case 10: case 12:	if(_jour > 0 && _jour <= 31) { return true; } else { return false; }
		
		case 2:	if (annee % ANNEE_BISEX_4 == 0)
					if (annee % ANNEE_BISEX_100 == 0)
						if (annee % ANNEE_BISEX_400 == 0)				
							if(_jour > 0 && _jour <= 29) { return true; } else { return false; }
						else											
							if(_jour > 0 && _jour <= 28) { return true; } else { return false; }
					else												
						if(_jour > 0 && _jour <= 29) { return true; } else { return false; }
				else													
					if(_jour > 0 && _jour <= 28) { return true; } else { return false; }
		
		case 4:	case 6:	case 9:	case 11:	if(_jour > 0 && _jour <= 30) { return true; } else { return false; }
		}
		
		return false; //Normalement, la méthode ne devrait pas se rendre là mais éclipse m'affiche une erreur sinon.
	}
